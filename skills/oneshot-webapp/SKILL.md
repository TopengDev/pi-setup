...
Take a brief and, in a SINGLE session, produce a polished, real-feeling web app or landing page and deploy it live at `https://<slug>.topengdev.com`. This codifies the proven ProjectAlpha / `recruitco-ops-pm` / ProjectBeta builds (2026-05-29).
...
4. **SHIP FAST — ACT, DON'T DELIBERATE.** Cap your thinking. Move in concrete, visible steps: get a working slice on screen, then refine the running app. Do NOT burn long high-effort thinking cycles deliberating architecture or "the perfect approach" — the ProjectAlpha worker stalled in 10–18 min thinking blocks and had to be interrupted. If you catch yourself planning instead of building, STOP and write code. Iterating a running app is faster than thinking your way to perfection.
...
- **Slug** — derive a clean subdomain from the app/company name: lowercase, hyphenated, no spaces, short. e.g. "ProjectAlpha for RecruitCo" → `recruitco`; "Acme Invoicing" → `acme-invoicing`. Final URL = `https://<slug>.topengdev.com`.
...
When you write the report, note this override explicitly (as the ProjectAlpha build did). It's intentional, not an oversight.
...
Next.js (App Router) + TypeScript + Tailwind + shadcn/ui. Mirror the proven `recruitco-ops-pm` setup:
...
## Proven gotchas checklist (from ProjectAlpha / recruitco-ops-pm / ProjectBeta — pre-empt these)
